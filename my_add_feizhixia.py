...
def feizhixia_main(logG,entry, path, db, cursor):
    # 文件是否齐全检查过程
    ...
    with os.scandir(path) as it:
        for entry in it:
            if not entry.name.startswith('.') and entry.is_dir() and entry.name != "logs" and entry != None:
                ...
                # 当前读取的文件夹所对应的板块存在
                if whetherExistData:
                    provinceRSid = whetherExistData[1]
                    if logG:
                        print("当前读取文件加所对应板块已经存在，板块id: %s" % provinceRSid)
                # 当前读取的文件夹所对应的板块不存在
                else:
                    provinceRSid = createReadsort(dirName1, cursor, db, label=dirName1)
                    if provinceRSid == False:
                        print("省级数据错误")
                        exit(0)
                    else:
                        if logG:
                            print("创建省级板块成功，板块id:%s" % provinceRSid)
                    ...
                    for entry2 in it2:
                        if not entry2.name.startswith('.') and entry2.is_dir():
                            ...
                            # 当前读取的文件夹所对应的板块不存在
                                ...
                                cityRSid = createReadsort(dirName2, cursor, db, pid=provinceRSid,label=dirName2)
                                if cityRSid == False:
                                ...
                                for entry3 in it3:
                                    ...
                                    for teacher in teacherTable:
                                        ...
                                        # 如果老师不存在
                                            ...
                                            teacherId = createTeacher(teacherInfor[0], teacherInfor[1], schoolId, cursor, db)
                                    ...
                                    for line in teacherCourseTable:
                                        ...
                                        # 当前的年级板块不存在的情况
                                            ...
                                            gradeRSid = createReadsort(gradeName, cursor, db, pid=schoolRSid, allowgroupids=stugroupId.__str__(), label=label)
                                            if logG:
                                        ...
                                        # 当前班级板块不存在的情况
                                            ...
                                            # 创建班级板块
                                            classRSid = createReadsort(className, cursor, db, pid=gradeRSid, allowgroupids=stugroupId.__str__(), label=label)
                                            if logG:
                                            ...
                                            # 创建班级
                                            classId = createClass(className, classRSid, stugroupId, courseName, gradeId, schoolId, cursor, db)
                                            if logG:
                                            ...
                                            if courseReadSortId == None:
                                                ...
                                                # 如果当前找到的课程板块不为空
                                                # 获取当前板块下的所有帖子
                                                sql = "SELECT title,content FROM yzbc.yz_read where sortid  = %s " % courseReadSortId[0]
                                                cursor.execute(sql)
                                        ...
                                        if teacherName in teacherQXTabel:
                                            # 为老师添加班级
                                            teacherClassId[teacherName] = teacherClassId[teacherName] + "," + classId.__str__()
                                            # 如果年级板块是新建的，则说明，这位老师负责的不同班级在不同年级
                                            if isgradenew:
                                                teacherQXTabel[teacherName] = teacherQXTabel[teacherName] + "," + gradeRSid.__str__() + "," + classRSid.__str__()
                                            # 如果年级板块不是新建的，则说明，这位老师负责的不同班级在同一年级，在权限设置时，就不再需要添加年级板块的权限
                                            else:
                                                teacherQXTabel[teacherName] = teacherQXTabel[teacherName] + "," + classRSid.__str__()
                                        ...
                                        # 如果在字典中不存在老师的名字，则说明是第一次进行当前老师负责班级数据录入
                                            ...
                                            # 为老师添加权限
                                            teacherQXTabel[teacherName] = provinceRSid.__str__()+','+cityRSid.__str__()+','+schoolRSid.__str__()+','+gradeRSid.__str__()+','+classRSid.__str__()
